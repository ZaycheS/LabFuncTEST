...
createStudent name surname app_suiteId conn = withTransaction conn (create' name surname app_suiteId)

create' name surname app_suiteId conn = do
  changed <- run conn query [SqlString name, SqlString surname, SqlInteger app_suiteId]
  result <- quickQuery' conn lastId []
  let rows = map Common.convRow result
  return $ last rows
  where
    query = "insert into hs1_student (name, surname, app_suiteId) values (?, ?, ?)"
    lastId = "select max(id) from hs1_student"
...
readStudent conn id = do
  ...
  where
    query = "select * from hs1_student where id = ?"
...
readAllStudents conn = do
  ...
  where
    query = "select * from hs1_student order by id"
...
update' uid name surname sid conn = do
  ...
  return $ last rows
  where
    query = "update hs1_student set name = ?, surname = ?, app_suiteId = ? where id = ?"
    newValue = "select id, name, surname, app_suiteId from hs1_student where id = ?"
...
delete' id conn = do
  ...
  return $ changed == 1
  where
    query = "delete from hs1_student where id = ?"
