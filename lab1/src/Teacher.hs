...
create' name surname conn = do
  ...
  return $ last rows
  where
    query = "insert into hs1_teacher (name, surname) values (?, ?)"
    lastId = "select max(id) from hs1_teacher"
...
readTeacher conn id = do
  ...
  where
    query = "select * from hs1_teacher where id = ?"
...
readAllTeachers conn = do
  ...
  where
    query = "select * from hs1_teacher order by id"
...
update' uid name surname conn = do
  ...
  return $ last rows
  where
    query = "update hs1_teacher set name = ?, surname = ? where id = ?"
    newValue = "select id, name, surname from hs1_teacher where id = ?"
...
delete' id conn = do
  ...
  return $ changed == 1
  where
    query = "delete from hs1_teacher where id = ?"
